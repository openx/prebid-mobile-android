...
public class RewardedAdWrapper extends FullScreenContentCallback
    ...
    static RewardedAdWrapper newInstance(Context context, String gamAdUnitId, GamAdEventListener eventListener) {
        try {
            ...
            PbLog.error(TAG, Log.getStackTraceString(throwable));
        }
        return null;
    }
    ...
    public void show(Activity activity) {
        if (mRewardedAd == null) {
            PbLog.error(TAG, "show: Failed! Rewarded ad is null.");
            return;
        }
    ...
    private boolean metadataContainsAdEvent() {
        try {
            if (mRewardedAd == null) {
                PbLog.debug(TAG, "metadataContainsAdEvent: Failed to process. RewardedAd is null.");
                return false;
            }
