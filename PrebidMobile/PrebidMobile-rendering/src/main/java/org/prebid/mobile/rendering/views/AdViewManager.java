...
public class AdViewManager implements CreativeViewListener, TransactionManagerListener {
    ...
    public void onFetchingFailed(AdException exception) {
        PbLog.error(TAG, "There was an error fetching an ad " + exception.toString());
    ...
    public void creativeInterstitialDidClose(AbstractCreative creative) {
        PbLog.debug(TAG, "creativeInterstitialDidClose");
    ...
    public void creativeDidComplete(AbstractCreative creative) {
        PbLog.debug(TAG, "creativeDidComplete");
    ...
    public void hide() {
        if (mCurrentCreative == null) {
            PbLog.warn(TAG, "Can not hide a null creative");
            return;
        }
    ...
    public void setAdVisibility(int visibility) {
        if (mCurrentCreative == null) {
            PbLog.debug(TAG, "setAdVisibility(): Skipping creative window focus notification. mCurrentCreative is null");
            return;
        }
    ...
    public void addObstructions(InternalFriendlyObstruction... friendlyObstructions) {
        if (friendlyObstructions == null || friendlyObstructions.length == 0) {
            PbLog.debug(TAG, "addObstructions(): Failed. Obstructions list is empty or null");
            return;
        }

        if (mCurrentCreative == null) {
            PbLog.debug(TAG, "addObstructions(): Failed. Current creative is null.");
            return;
        }
    ...
    public void show() {
        if (!isCreativeResolved()) {
            PbLog.debug(TAG, "Couldn't proceed show(): Video or HTML is not resolved.");
            return;
        }
        AbstractCreative creative = mTransactionManager.getCurrentCreative();
        if (creative == null) {
            PbLog.error(TAG, "Show called with no ad");
            return;
        }
    ...
    private void handleCreativeDisplay() {
        ...
        if (creativeView == null) {
            PbLog.error(TAG, "Creative has no view");
            return;
        }
    ...
    private void handleAutoDisplay() {
        ...
        if (mAdViewListener != null && (mCurrentCreative != null && isAutoDisplayOnLoad())) {
            ...
            PbLog.info(TAG, "AdViewManager - Ad will be displayed when show is called");
        }
    }

    private void addHtmlInterstitialObstructions(ViewGroup rootViewGroup) {
        if (rootViewGroup == null) {
            PbLog.debug(TAG, "addHtmlInterstitialObstructions(): rootViewGroup is null.");
            return;
        }
    ...
    private void processTransaction(Transaction transaction) {
        ...
        try {
            ...
            PbLog.error(TAG, "adLoaded failed: " + Log.getStackTraceString(e));
        }
