...
public class PrebidRenderingSettings {
    ...
    public static void setBidServerHost(Host host) {
        if (host == null) {
            PbLog.error(TAG, "setBidServerHost: Error. Can't assign a null host.");
            return;
        }
    ...
    private static void initializeLogging() {
        PbLog.setLogLevel(logLevel.getValue());//set to the publisher set value
        increaseTaskCount();
    }
    ...
    static void increaseTaskCount() {
        if (INIT_SDK_TASK_COUNT.incrementAndGet() >= MANDATORY_TASK_COUNT) {
            ...
            PbLog.debug(TAG, "Prebid Rendering SDK " + SDK_VERSION + " Initialized");
