...
public abstract class AdBaseDialog extends Dialog {
    ...
    private final FetchPropertiesHandler.FetchPropertyCallback mExpandPropertiesCallback = new FetchPropertiesHandler.FetchPropertyCallback() {
        ...
        public void onError(Throwable throwable) {
            PbLog.error(TAG, "ExpandProperties failed: " + Log.getStackTraceString(throwable));
        }
    };
    ...
    protected void lockOrientation() {
        Activity activity = getActivity();
        if (activity == null) {
            PbLog.error(TAG, "lockOrientation failure. Activity is null");
            return;
        }
    ...
    protected Activity getActivity() {
        try {
            ...
            PbLog.error(TAG, "Context is not an activity");
            return null;
        }
    }
    ...
    protected void addCloseView() {
        if (mAdViewContainer == null) {
            PbLog.error(TAG, "Unable to add close button. Container is null");
            return;
        }
        ...
        if (mCloseView == null) {
            PbLog.error(TAG, "Unable to add close button. Close view is null");
            return;
        }
    ...
    private void handleExpandPropertiesResult(String expandProperties) {
        if (mWebViewBase == null || mWebViewBase.getMRAIDInterface() == null) {
            PbLog.debug(TAG, "handleExpandPropertiesResult: WebViewBase or MraidInterface is null. Skipping.");
            return;
        }
    ...
    private void initOrientationProperties() {
        ...
        try {
            ...
            PbLog.error(TAG, "Failed to get the orientation details from JSON for MRAID: " + Log.getStackTraceString(e));
        }
    ...
    private static class OnDialogShowListener implements OnShowListener {
        ...
        public void onShow(DialogInterface dialog) {
            ...
            if (adBaseDialog == null) {
                PbLog.debug(TAG, "onShown(): Error notifying show listeners. AdBaseDialog is null.");
                return;
            }
