...
public class MraidExpand {
    ...
    private void performExpand(String url, CompletedCallBack completedCallBack) {
        ...
        if (context == null) {
            PbLog.error(TAG, "Context is null");
            return;
        }
        ...
        uiHandler.post(() -> {
            try {
                ...
                if (isContainerStateInvalid(state)) {
                    PbLog.debug(TAG, "handleExpand: Skipping. Wrong container state: " + state);
                    return;
                }
                ...
                PbLog.error(TAG, "Expand failed: " + Log.getStackTraceString(e));
            }
        });
    }
    ...
    void showExpandDialog(Context context, CompletedCallBack completedCallBack) {
        if (!(context instanceof Activity) || ((Activity) context).isFinishing()) {
            PbLog.error(TAG, "Context is not activity or activity is finishing, can not show expand dialog");
            return;
        }
