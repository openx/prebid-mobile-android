        ...
        uiHandler.post(() -> {
            try {
                ...
                if (isContainerStateInvalid(state)) {
                    PbLog.debug(TAG, "closeThroughJS: Skipping. Wrong container state: " + state);
                    return;
                }
                ...
                PbLog.error(TAG, "closeThroughJS failed: " + Log.getStackTraceString(e));
            }
        });
    }
    ...
    private void makeViewInvisible() {
        ...
        handler.post(() -> {
            if (mWebViewBase == null) {
                PbLog.error(TAG, "makeViewInvisible failed: webViewBase is null");
                return;
            }
