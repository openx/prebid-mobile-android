...
public class ScreenMetricsWaiter {
    ...
    void queueMetricsRequest(
        ...
        PbLog.debug(TAG, "New request queued. Queue size: " + mWaitRequestQueue.size());
    }

    void finishAndStartNextRequest() {
        mWaitRequestQueue.removeFirst();
        WaitRequest firstInQueueRequest = mWaitRequestQueue.peekFirst();
        PbLog.debug(TAG, "Request finished. Queue size: " + mWaitRequestQueue.size());
            ...
            public void run() {
                ...
                for (final View view : mViews) {
                    ...
                    if (view.getHeight() > 0 || view.getWidth() > 0 || mIsAnswerRequired || isTwoPart) {
                        countDown();
                        PbLog.debug(TAG, "Get known metrics for: " + view.getClass().getSimpleName() + ", h: " + view.getHeight() + ", w: " + view.getWidth());
                        continue;
                    }
                    ...
                    PbLog.debug(TAG, "Create listener for: " + view.getClass().getSimpleName());
                    view.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
                        @Override
                        public boolean onPreDraw() {
                            PbLog.debug(TAG, "Get metrics from listener for: " + view.getClass().getSimpleName() + ", h: " + view.getHeight() + ", w: " + view.getWidth());
