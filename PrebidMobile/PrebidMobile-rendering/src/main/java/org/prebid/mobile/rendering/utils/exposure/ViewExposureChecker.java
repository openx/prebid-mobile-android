...
public class ViewExposureChecker {
    ...
    public ViewExposure exposure(View view) {
        if (view == null) {
            PbLog.debug(TAG, "exposure: Returning zeroExposure. Test View is null.");
            return null;
        }
        ...
        PbLog.debug(TAG, "exposure: visitParent " + visitParent + " collapseBox " + collapseBoundingBox);
    ...
    private void fragmentize(Rect valueRect, Rect aroundRect, List<Rect> destList) {
        ...
        if (!isRectTrimmed) {
            PbLog.debug(TAG, "fragmentize: Error. Rect is not trimmed");
            return;
        }
    ...
    private Rect convertRect(Rect fromRect, View fromView, View toView) {
        if (fromRect == null || fromView == null || toView == null) {
            PbLog.debug(TAG, "convertRect: Failed. One of the provided param is null. Returning empty rect.");
            return new Rect();
        }
