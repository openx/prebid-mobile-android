...
public class MraidEventHandlerNotifierRunnable implements Runnable {
    ...
    public void run() {
        ...
        if (htmlCreative == null || webViewBase == null) {
            PbLog.debug(TAG, "Unable to pass event to handler. HtmlCreative or webviewBase is null");
            return;
        }
        ...
        if (jsExecutor == null) {
            PbLog.debug(TAG, "Unable to executeNativeCallComplete(). JsExecutor is null.");
            return;
        }
